import * as orderService from "../services/orderSerivce.js";
...
const getMyBuyingOrders = asyncHandler(async (req, res) => {
    const orders = await orderService.getMyBuyingOrders(req.user._id);
...
const getMySellingOrders = asyncHandler(async (req, res) => {
    const orders = await orderService.getMySellingOrders(req.user._id);
...
const generateOtp = asyncHandler(async (req, res) => {
  const { rawOtp, otpExpiresAt } = await orderService.generateOtp({
    orderId: req.params.id,
  ...
  return res.status(200).json(
    new ApiResponse(
      200,
      {
        otp: rawOtp,         // shown to seller once — never stored raw
...
const verifyOtp = asyncHandler(async (req, res) => {
  ...
  const order = await orderService.verifyOtp({
    orderId: req.params.id,
    buyerId: req.user._id,
    enteredOtp: otp,
  });
